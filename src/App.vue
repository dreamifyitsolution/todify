...
<template>
    <div class="flex flex-row justify-center space-x-6 pt-10">
        <div v-for="column of data.columns" :key="column.name" class="rounded-md shadow-md bg-slate-100 py-4 px-4 w-1/4 space-y-4">
            <div>{{ column.name }}</div>
            <div v-for="item of column.list" :key="item.id" class="bg-slate-50 rounded-md py-2 px-3 shadow-md">
                {{ item.description }}
            </div>
            <div class="flex flex-col items-end">
                <form @submit.prevent="addItem($event, column)">
                    <input type="text" name="description" class="w-full border px-3 py-2 rounded-md mb-2 shadow-sm" autocomplete="off" />
                    <button>Add</button>
                </form>                
            </div>
        </div>       
